...
export function PatientAuthScreen({ onBack }: { onBack?: () => void }) {
    ...
    <main className="flex flex-1 items-center justify-center px-6 py-12">
      <section className="w-full max-w-md border border-border bg-card/60 p-8 shadow-[0_24px_80px_-40px_var(--primary)] sm:p-10">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight">Patient</h1>
          ...
          <form className="mt-6 flex w-full flex-col gap-4 text-left" onSubmit={handleEmailAuth}>
            ...
            <Button type="submit" disabled={busy || !isFirebaseConfigured} className="h-12 rounded-none">
          ...
          {!isFirebaseConfigured && (
            <p className="mt-4 text-xs text-risk">Add Firebase values to `.env.local` to enable sign-in.</p>
          )}
          ...
          <div className="mt-8 flex items-center gap-2 text-xs text-muted-foreground/70">
            <ShieldCheck className="h-3.5 w-3.5 text-primary" /> Your account stays private and secure.
          </div>
        </div>
      </section>
    </main>
